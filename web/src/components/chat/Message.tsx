...
const Message: React.FC<MessageProps> = ({ 
  ...
  return (
    <div 
      className={`flex w-full mb-4 sm:mb-6 group ${isUserMessage ? "justify-end" : "justify-start"} px-2 sm:px-0 animate-slide-in`}
      style={style}
    >
      <div className={`flex items-end gap-2 sm:gap-3 max-w-full ${isUserMessage ? 'flex-row-reverse' : 'flex-row'}`}>
        {/* Avatar */}
        <div className={`w-6 h-6 sm:w-8 sm:h-8 rounded-full flex-shrink-0 flex items-center justify-center transition-all duration-300 ${
          ...
          {isUserMessage ? 
            ...
            <Bot className="w-3 h-3 sm:w-4 sm:h-4 text-gray-300" />
          }
        </div>

        {/* Message Bubble & Content */}
        <div className={`max-w-[85%] sm:max-w-md md:max-w-lg p-3 sm:p-4 rounded-2xl shadow-md transition-all duration-300 ${
          isUserMessage 
